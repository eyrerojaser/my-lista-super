...
(function () {
  ...
  function updateSums() {
    ...
    row.classList.toggle("warn", diff < 0);
    $("rcDiffLbl").textContent = diff < 0 ? "Los productos suman más que el total" : "Impuestos y otros";
  ...
  $("pcItemsBtn").onclick = async () => {
    ...
    openReview({ store: rec.store, date: rec.date, items: rec.items || [], total: rec.cents }, false, u);
  };
